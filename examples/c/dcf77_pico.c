 ...
 *  This version uses PicoOS
 *  Compile with "cl65 -t mycpu dcf77_pico.c picolib.lib -o dcf77_pico"
 *
...
void startProgram(void)
{
	...
	/* increase the shell priority so shell becomes usable */
	setShellPriority(curprio);

	/* try to increase current priority */
	posTaskSetPriority(self, curprio + 1);
	
	/* allocate a flag object */
    newSecFLG = posFlagCreate();
    if (newSecFLG == NULL)
	{
    ...
    while (1)
	{
		...
		flagNo = posFlagGet(newSecFLG, POSFLAG_MODE_GETSINGLE);
